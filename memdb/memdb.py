...
class Table:
    def __init__(self, member_list: List[Field], key_set_list: List[list]) -> None:
        ...
        self.member_name_set = [x.name for x in member_list]
        self.key_info_list = []
        self.data: list[Data] = []
        self.index_map = {}
        for key_set in key_set_list:
            key_set_hash = get_hash(key_set)
            self.key_info_list.append(
                {"key_set": key_set, "key_set_hash": key_set_hash})
            if key_set_hash not in self.index_map:
                self.index_map[key_set_hash] = {}
        # unique
        self.unique_key_set = key_set_list[0]
        self.unique_key_set_hash = get_hash(self.unique_key_set)
        self.unique_index = self.index_map[self.unique_key_set_hash]
    ...
    def __get_unique(self, key_value: dict) -> Data:
        unique_value_hash = self.__get_value_hash(
            self.unique_key_set, key_value)
        if unique_value_hash not in self.unique_index:
            return None

        data_value_list: list[Data] = self.unique_index[unique_value_hash]
        for data_value in data_value_list:
            found = True
            for key in self.unique_key_set:
    ...
    def insert(self, *args, **kwargs) -> bool:
        ...
        if set(key_value.keys()) != set(self.member_name_set):
            return False
